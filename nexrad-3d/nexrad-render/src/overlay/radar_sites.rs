//! Radar site markers: one clickable icon per NEXRAD site on the basemap.
...
use super::{OverlayLayerId, SiteClickNotifier};

const WORLD_ORIGIN_LAT: f64 = 36.0;
const WORLD_ORIGIN_LNG: f64 = -98.0;
const SITE_MARKER_RADIUS: f32 = 15_000.0; // meters
const SITE_MARKER_HEIGHT: f32 = 500.0; // above ground to avoid z-fight
const SITE_MARKER_COLOR: Vec4 = Vec4::new(0.0, 0.8, 1.0, 1.0); // cyan

/// Material for the radar site marker (ring + dot, unlit).
...
impl Material for SiteMarkerMaterial {
    fn fragment_shader() -> ShaderRef {
        "embedded://nexrad_render/overlay/shaders/site_marker.wgsl".into()
    }
...
/// Marks an entity as a radar site marker; used by the click observer to read site_id.
        ...
        entity_commands.observe(
            |evt: On<Pointer<Click>>,
             ...
             notifier: Res<SiteClickNotifier>| {
                log::info!("pointer click observer fired for {:?}", evt.entity);
                if let Ok(marker) = markers.get(evt.entity) {
                    log::info!("site marker clicked: {}", marker.site_id);
                    notifier.notify(&marker.site_id);
                } else {
                    log::warn!("click observer: no RadarSiteMarker on {:?}", evt.entity);
                }
            },
        );
    }
